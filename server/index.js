...
app.get("/api/my/submissions", requireAuth, async (req, res) => {
  try {
    ...
    const submissions = rows.map((row) => ({
      ...
      country: row.country,
      city: row.city,
      // Everything the applicant filled in on the public form, so the portal can
      // show it back to them instead of scattering it across screens.
      documentType: row.document_type ?? null,
      documentNumber: row.cpf ?? null,
      occupation: row.occupation ?? null,
      experience: row.experience ?? null,
      session: row.session ?? null,
      riskPerDay: row.risk_per_day ?? null,
